...
export class LocationListComponent implements OnInit {
  ...
  // make the user log in to manage lists
    if (!this.user.authData) {
      ...
      this.dataSvc.getList(this.dataSvc.LOCATION_TABLE_NAME)
      .then((list) => {
  ...
  submitRequest(form : NgForm) : void {
    ...
    this.dataSvc.updateList(this.dataSvc.LOCATION_TABLE_NAME, this.itemName, action, parseInt(this.selectedItem))   // send the update
    .then((success) => {
      this.utilSvc.setUserMessage(msgId, msg);
      this.utilSvc.displayUserMessages();
      this.resetForm(form);
      this.dataSvc.getList(this.dataSvc.LOCATION_TABLE_NAME)  // re-read the list
      .then((list) => {
