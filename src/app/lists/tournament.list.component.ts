...
import { DataSvc } from '../model/dataSvc'
    ...
    // COMPONENT for MANAGE TOURNAMENTS feature
...
export class TournamentListComponent implements OnInit {
  ...
  // make the user log in to manage lists
    if (!this.user.authData) {
      ...
      this.utilSvc.setCurrentHelpContext("ManageTournaments"); // note current state
      this.utilSvc.displayUserMessages();;
      this.dataSvc.getList(this.dataSvc.TOURNAMENT_TABLE_NAME)
      .then((list) => {
  ...
  submitRequest(form : NgForm) : void {
    ...
    this.dataSvc.updateList(this.dataSvc.TOURNAMENT_TABLE_NAME, this.itemName, action, parseInt(this.selectedItem))   // send the update
    .then((success) => {
      this.utilSvc.setUserMessage(msgId, msg);
      this.utilSvc.displayUserMessages();
      this.resetForm(form);
      this.dataSvc.getList(this.dataSvc.TOURNAMENT_TABLE_NAME)  // re-read the list
      .then((list) => {
      ...
      .catch((error) => {
          this.utilSvc.displayThisUserMessage("errorReadingTournamentList");
          this.working = false;
      });
    })
    .catch((error) => {
        this.utilSvc.displayThisUserMessage("errorUpdatingTournamentList");
        this.resetForm(form);
        this.working = false;
    });
  }
